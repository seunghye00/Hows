...
export const FindPw = ({ onBack }) => {
    ...
    const handleSubmitNewPw = (newPw) => {
        // 여기에서 새로운 비밀번호를 서버로 보내는 로직 작성
        alert(`새 비밀번호: ${newPw} 제출됨.`);
        // 예시로 비밀번호를 서버에 전송하는 코드
        axios.post(`${host}/auth/changePassword`, { newPw })
            .then(() => {
            ...
            {!showNewPw ? (
                !isVerify ? (
                    <div className={styles.findPwBox}>
                        ...
                        <button className={styles.btn} onClick={handleSendEmail}>인증번호 발송</button>
